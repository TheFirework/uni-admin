...
1. **静态路由文件的维护责任归属**: 由**前端团队**负责更新 `modules/*.ts`，后端团队提供 API 文档作为参考；建立"前端主导、后端审核"的协作流程——前端根据后端 API 文档编写静态路由结构，后端在 API 变更时同步通知前端更新；代码评审时需同时通过前后端双方的审核。
2. **缓存 TTL 的生产环境最佳值**: 30 分钟是否合适？还是应该根据业务特点调整（如 10 分钟或 1 小时）？
3. **权限轮询接口的后端支持**: **决策：复用现有 `/system/menus` 接口**，在响应中增加 `version` 字段（如 `{ menus: [...], version: "v1.2.3" }`），避免新建接口；前端缓存时同时存储 version，轮询时仅对比 version 是否变化；若 version 变更则重新请求完整菜单数据并刷新缓存。
4. **预加载的颗粒度**: 当前方案是预加载整个 Vue 组件（包含模板+样式+脚本），是否需要细化到仅预加载脚本部分（SplitChunks）？

...
- [x] 1.1 创建 `src/stores/app.store.ts` — 全局应用状态 Pinia Store
...
- [x] 1.2 创建 `src/components/PageLoading.vue` — 全屏 Loading 组件
...
- [x] 1.3 修改 `src/main.ts` — 集成页面类型检测逻辑
...
- [x] 2.1 修改 `src/App.vue` — 集成全屏 Loading 条件渲染
  - 导入 `useAppStore` 和 `PageLoading` 组件
  - 在 `<router-view>` 外层添加条件判断：
    - 当 `appStore.actualIsFullLoading === true` 时渲染 `<PageLoading />`
    - 否则正常渲染 `<router-view>`（保持现有的 transition 和 layout 逻辑）
  - 确保 Loading 层级正确（不干扰后续的 BasicLayout 渲染）

- [x] 2.2 修改 `src/layouts/BasicLayout.vue` — 集成骨架屏 + 移除重复加载
  - 导入 `useAppStore` 和 `LayoutSkeleton` 组件
  - 在 `<el-main class="content-main"> 内部添加条件渲染：
    - 当 `appStore.actualIsRouteLoading === true` 时渲染 `<LayoutSkeleton :model-value="true" />`
...
- [x] 2.3 修改 `src/router/guards/layers/dynamicRoute.ts` — 对接 App Store 控制 Loading 状态
  - 在文件顶部导入 `useAppStore`
  - 在 `dynamicRouteMiddleware` 函数中：
    - **进入时**（开始加载动态路由）：在 `await menuStore.fetchMenus()` 之前调用
      - `appStore.setFullLoading(true)`
      - `appStore.setRouteLoading(true)`
    - **成功完成时**（fetchMenus 之后）：调用
...
- [x] 3.4 代码质量检查 ✅
  - 运行 `pnpm typecheck`（或等效命令）→ 确认无新增 TypeScript 类型错误 ✅ (预先存在的错误未增加)
  - 运行 `pnpm lint`（或等效命令）→ 确认无新增 ESLint 报错 ✅ (web 项目无配置文件，server 错误为预先存在)

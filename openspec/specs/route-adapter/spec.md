...
定义多模式路由数据源适配规范，支持 frontend/backend/mixed 三种路由数据源模式的无缝切换，实现开发环境独立运行、生产环境动态权限控制、测试环境灰度发布的灵活适配。
...
- **Key Files**:
  - [router.config.ts](../../../apps/web/src/config/router.config.ts)
  - [RouteAdapter.ts](../../../apps/web/src/router/adapters/RouteAdapter.ts)
  - [types.ts](../../../apps/web/src/router/routes/types.ts)
